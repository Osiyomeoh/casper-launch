...
export default function AppLayout({
  ...
  return (
    <div className="bg-[#011230] text-[#d8e2ff] font-sans min-h-screen flex">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col h-screen sticky top-0 bg-[#091b39] border-r border-[rgba(100,255,218,0.12)] w-56 lg:w-64 shrink-0 z-40">
        <div className="px-5 py-6">
          <Link href="/">
            <span className="text-xl font-bold text-[#FF0000] cursor-pointer">CasperLaunch</span>
          </Link>
          <p className="text-[10px] font-mono text-[#abb9d6] mt-1 uppercase tracking-widest">Asset Manager</p>
        </div>
          ...
          {NAV.map((item) => {
            ...
            return (
              <Link key={item.label} href={item.href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${active ? "text-[#64FFDA] font-bold bg-[#192a48] border-r-2 border-[#64FFDA]" : "text-[#abb9d6] hover:bg-[#192a48]/60 hover:text-[#d8e2ff]"}`}>
        ...
        <div className="px-4 py-5 border-t border-[rgba(100,255,218,0.12)] space-y-3">
          <Link href="/chat">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            {casperWallet.isInstalled ? (
              casperWallet.isConnected ? (
                ...
                <button onClick={casperWallet.connect} disabled={casperWallet.loading}
                  ...
                  {casperWallet.loading ? "..." : "Connect Wallet"}
                </button>
              )
            ) : null}
            ...
            <button onClick={() => logout()} title="Sign out" className="hidden md:flex w-8 h-8 items-center justify-center rounded-lg bg-[#112240] hover:bg-[#192a48] transition-colors">
